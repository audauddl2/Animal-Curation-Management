...
<div class="InnerWrap">
	...
	<h1>QnA</h1>
			...
			<div class="InnerTable2" style="width:100%, height:200px; overlfow:auto; text-align:center;">
	    		...
	    		<tbody>
	   				 ...
	   				 <div>
		   				 <c:choose>
	               			 ...
	               			 <c:forEach items="${qnaList}" var="qnaList" varStatus="rowcnt">
	                  		 	<tr>
	                        		<td>공지</td>
	                        		<td><a href="/board/qna/qnaDetail?bbsIdx=${qnaList.bbsIdx}">${qnaList.bbsTitle}</a></td>
	                        		<td>${qnaList.writeDate}</td>
	                        		<td>${qnaList.bbsCnt}</td>
	                        		<td><form action="/delete?bbsIdx=${qnaList.bbsIdx}" method="POST">
	                        		<input type="submit" value="삭제" name="qnaDelete"/></form></td>
	                    		</tr>
	                		</c:forEach>
	                		</c:when>
	            			<c:otherwise>
				...
				<div><a href="/board/qna/qnaWrite"><input type="button" value="등록"/></a></div>
			</div>
		</div>
	</div>
</body>
