...
<div class="InnerWrap">
	...
	<h1>게시판 목록</h1>
			...
			<form class="InnerContent" action="/board/column/columnSearch" name="search" method="post">
				...
				<select name="keyField" id="keyField" size="1" style="padding: 2px">
					...
					<option value="columnTitle">제목</option>
					<option value="writer">이름</option>
					<option value="columnContent">내용</option>
				</select>
	...
	<div class="InnerTable2" style="width:100%, height:200px; overlfow:auto; text-align:center;">
		    ...
		    <c:choose>
	            <c:when test="${fn:length(noticeList) > 0}">
	                <c:forEach items="${noticeList}" var="noticeList" varStatus="rowcnt">
	                    <tr style="background:#7c98cc">
	                        <td>공지</td>
	                        <td><a href="/board/notice/noticeDetail?bbsIdx=${noticeList.bbsIdx}">${noticeList.bbsTitle}</a></td>
	                        <td>${noticeList.writeDate}</td>
	                        <td>${noticeList.bbsCnt}</td>
	                        <td><a href="/delete?bbsIdx=${noticeList.bbsIdx}"><input type="button" value="삭제"/></a></td>
	                    </tr>
	                </c:forEach>
	            </c:when>
	            <c:otherwise>
	    ...
	    <!-- 게시글 출력 -->
	        <c:choose>
	            <c:when test="${fn:length(columnSearch) > 0}">
	                <c:forEach items="${columnSearch}" var="columnSearch" varStatus="rowcnt">
	                    <tr>
	                        <td>${columnSearch.bbsIdx}</td>
	                        <td><a href="/board/column/columnDetail?bbsIdx=${columnSearch.bbsIdx}">${columnSearch.bbsTitle}</a></td>
	                        <td>${columnSearch.writeDate}</td>
	                        <td>${columnSearch.bbsCnt}</td>
	                        <td><a href="/delete?bbsIdx=${columnSearch.bbsIdx}"><input type="button" value="삭제"/></a></td>
	                    </tr>
	                </c:forEach>
	            </c:when>
	            <c:otherwise>
		...
		<div><a href="/board/column/columnWrite"><input type="button" value="등록"/></a></div>
	</div>
	</div>
</div>
</body>
