...
<div class="InnerWrap">
	...
	<div class="InnerTable2" style="width:100%, height:200px; overlfow:auto; text-align:center;">
	    ...
	    <!-- 게시글 출력 -->
	        <c:choose>
	            <c:when test="${fn:length(columnList) > 0}">
	                <c:forEach items="${columnList}" var="columnList" varStatus="rowcnt">
	                    <tr>
	                        <td>${columnList.bbsIdx}</td>
	                        <td><a href="/board/column/columnDetail?bbsIdx=${columnList.bbsIdx}">${columnList.bbsTitle}</a></td>
	                        <td>${columnList.writeDate}</td>
	                        <td>${columnList.bbsCnt}</td>
	                        <td><form action="/delete?bbsIdx=${columnList.bbsIdx}" method="POST">
